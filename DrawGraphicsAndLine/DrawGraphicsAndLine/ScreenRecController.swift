            ...
            self.concat { (error, filepath) in
                if let err = error {
                    ...
                    if let fileP = filepath {
                        // 保存
                        self.save(with: fileP) { (error, exportpath) in
                            if let err = error {
                                // TmpFile 削除する
                                ...
                                // すべてSuccess
                                self.mListener?.onRecComplete(path: exportpath ?? "")
                            }
                        }
                    }
                }
...
extension ScreenRecController {
    ...
    // ③ save
    fileprivate func save(with exportFilePath: String, complete: ((_ error: ScreenRecError?, _ filePath: String?) -> Void)? = nil) {
